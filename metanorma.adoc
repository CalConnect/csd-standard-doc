...
Currently the Metanorma process is realised in the Ruby programming language, with the non-WYSIWYG text input expressed in Asciidoctor, and output generated in
...
NOTE: The _Basic Document_ model makes no provision for "ASCII art", the use of text instead of images. Models derived from the _Basic Document_ model do permit such use of text by extending the modelling of figure.
...
* Zero or more bibliographies (Standard References Sections: <<standardssection>>).
...
* Standard References Sections are a subclass of Standard Section. Like the References Sections of the _Basic Document_ model (<<basicsection>>), they are leaf notes which contain zero or more bibliographical references (as modelled in <<relaton>>).
...
* Definition Sections consist of one or more Definition lists (<<basiclist>>), and are used to define symbols and abbreviations used in the remainder of the document. They can also be used as glossaries, with simple definitions, in contrast to the more elaborate definitions given in Terms Sections.
* Terms Sections give elaborated definitions of terms used in the document; they are intended to align to the structure of terms in <<isoiecdir2>>. Term Sections consist of zero or more Terms; Terms in turn are composed of the following elements:
...
There is no difference in the modelling of blocks or data types between the _Basic Document_ model and the _Standards Document_ model.
