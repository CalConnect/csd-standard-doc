...
`notes`:: any notes (<<notes>>) whose scope is the entire section, rather than a specific block.
...
`notes`:: any notes (<<notes>>) whose scope is the block.
...
Paragraphs can contain any sequence of basic inline elements (<<inline>>), and optionally a text alignment (`alignment`). 
...
Tables are modelled consistently with HTML 5. They contain the following elements:
